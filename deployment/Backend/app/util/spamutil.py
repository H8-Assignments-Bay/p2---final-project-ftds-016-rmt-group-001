...
def __load_slangwords_stopwords():
    ...
    with open('./util/model/slangwords.txt', 'rt') as file_1:
        __slangwords = {}
        for line in file_1:
    ...
    with open('./util/model/random_forest.pkl', 'rb') as file_4:
...
def predict_spam(comments: pd.DataFrame):
    ...
    y_pred = __model.predict_proba(comments_vectorized)[:,1] >= 0.334375
